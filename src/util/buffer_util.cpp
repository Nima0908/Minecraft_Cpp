...
BufferUtil::BufferUtil(std::vector<uint8_t> data) : buffer_(std::move(data)), readPos_(0) {}
...
int32_t BufferUtil::readVarInt() {
    ...
    uint8_t byte;
    do {
        if (readPos_ >= buffer_.size()) throw std::runtime_error("VarInt read out of bounds");
...
uint16_t BufferUtil::readUInt16() {
    if (readPos_ + 2 > buffer_.size()) throw std::runtime_error("UInt16 read out of bounds");
...
uint8_t BufferUtil::readByte() {
    if (readPos_ >= buffer_.size()) throw std::runtime_error("Byte read out of bounds");
    return buffer_[readPos_++];
}

std::string BufferUtil::readString() {
    int length = readVarInt();
    if (readPos_ + length > buffer_.size()) throw std::runtime_error("String read out of bounds");
    std::string str(buffer_.begin() + readPos_, buffer_.begin() + readPos_ + length);
    readPos_ += length;
    return str;
}
...
std::vector<uint8_t> BufferUtil::readBytes(size_t length) {
    if (readPos_ + length > buffer_.size()) throw std::runtime_error("ByteArray read out of bounds");
    std::vector<uint8_t> out(buffer_.begin() + readPos_, buffer_.begin() + readPos_ + length);
    readPos_ += length;
    return out;
}

size_t BufferUtil::remaining() const {
    return buffer_.size() - readPos_;
}

const std::vector<uint8_t>& BufferUtil::data() const {
    return buffer_;
}
