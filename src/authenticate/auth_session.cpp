...
void sendJoinServerRequest(const std::string& accessToken,
    ...
    if (res != CURLE_OK) {
        mc::log(mc::LogLevel::ERROR, "Mojang join failed: " + std::string(curl_easy_strerror(res)));
    } else {
...
std::string getMinecraftUUID(const std::string& accessToken) {
    ...
    CURLcode res = curl_easy_perform(curl);
    if (res != CURLE_OK) {
        mc::log(mc::LogLevel::ERROR, "Failed to get profile: " + std::string(curl_easy_strerror(res)));
    } else {
    ...
    try {
        ...
        if (json.contains("id")) {
            mc::log(mc::LogLevel::DEBUG, "UUID parsed from profile: " + json["id"].get<std::string>());
            return json["id"];
        } else {
