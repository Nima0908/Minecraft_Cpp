...
AESCipher::AESCipher(const std::vector<uint8_t> &key) {
  ...
  mc::log(mc::LogLevel::DEBUG,
          "Initializing AES CFB8 cipher with 128-bit key...");
...
std::vector<uint8_t> AESCipher::encrypt(const std::vector<uint8_t> &data) {
  mc::log(mc::LogLevel::DEBUG,
          "Encrypting data block of size: " + std::to_string(data.size()));
  ...
  mc::log(mc::LogLevel::DEBUG,
          "Encrypted output length: " + std::to_string(outlen));
  return out;
}

std::vector<uint8_t> AESCipher::decrypt(const std::vector<uint8_t> &data) {
  mc::log(mc::LogLevel::DEBUG,
          "Decrypting data block of size: " + std::to_string(data.size()));
  std::vector<uint8_t> out(data.size());
  int outlen = 0;
  ...
  mc::log(mc::LogLevel::DEBUG,
          "Decrypted output length: " + std::to_string(outlen));
  return out;
}
