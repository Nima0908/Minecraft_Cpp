...
ReadBuffer::ReadBuffer(ByteArray data)
...
ByteArray ReadBuffer::readBytes(size_t len) {
    if (!ensure(len)) throw std::runtime_error("Read out of bounds");
...
uint8_t ReadBuffer::readByte() {
    if (!ensure(1)) throw std::runtime_error("Byte read out of bounds");
...
int32_t ReadBuffer::readVarInt() {
    ...
    uint8_t byte;
    do {
        if (!ensure(1)) throw std::runtime_error("VarInt read out of bounds");
        byte = data_[readPos_++];
...
uint16_t ReadBuffer::readUInt16() {
    if (!ensure(2)) throw std::runtime_error("UInt16 read out of bounds");
...
std::string ReadBuffer::readString() {
    int len = readVarInt();
    if (!ensure(len)) throw std::runtime_error("String read out of bounds");
...
size_t ReadBuffer::remaining() const {
    return data_.size() - readPos_;
}

const ByteArray& ReadBuffer::data() const {
    return data_;
}
