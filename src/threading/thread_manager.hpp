...
public:
    ...
    auto submitToPool(Func&& f, Args&&... args) -> std::future<decltype(f(args...))> {
...
private:
    ...
    void poolWorker() {
        while (running) {
            std::function<void()> task;
            {
                ...
                if (!running && poolQueue.empty()) return;
    ...
    std::unordered_map<std::string, std::queue<std::function<void()>>> dedicatedQueues;
    ...
    void startDedicatedThread(const std::string& name) {
        ...
        dedicatedCvs.emplace(name, std::condition_variable{});
        ...
        dedicatedThreads[name] = std::thread([this, name]() {
            ...
            while (running) {
                std::function<void()> task;
                {
                    ...
                    if (!running && dedicatedQueues[name].empty()) return;
